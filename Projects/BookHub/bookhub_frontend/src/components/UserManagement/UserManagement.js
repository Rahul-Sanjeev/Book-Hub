...
function UserManagement() {
  ...
  const handleSubmit = (e) => {
    ...
    if (editUser) {
      ...
      axios
        .put(`http://localhost:8000/api/users/${editUser.id}/`, userData)
        .then(() => {
          fetchUsers();
          setNewUser("");
          setEmail("");
    ...
    <div className="container" style={{ maxWidth: "800px", margin: "0 auto", padding: "20px" }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
        ...
        <input
          ...
          style={{ flex: 1, padding: "8px" }}
        />
        <button type="submit" style={{ padding: "8px 16px", backgroundColor: "green", color: "white" }}>
          ...
          {filteredUsers.map((user) => (
            <tr key={user.id}>
              ...
              <td style={{ border: "1px solid #ccc", padding: "8px", textAlign: "center" }}>
                <button onClick={() => handleEdit(user)} style={{ marginRight: "10px", padding: "5px 10px", backgroundColor: "cyan" }}>Edit</button>
                <button onClick={() => handleDelete(user.id)} style={{ padding: "5px 10px", backgroundColor: "red", color: "white" }}>Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
